...
export default async function getAndVerifyUser(req, method, connection) {
	...
	try {
		...
		if (session && session.user) {
			...
			let [calendarsId] = await connection.execute('SELECT calendar_id_public, calendar_name, calendar_color FROM calendars WHERE calendar_user_id = ?', [api.api_user_id]);

			user.calendarsId = calendarsId.map((item) => item.calendar_id_public);

			if (req.query.calendarsId && !user.calendarsId.includes(req.query.calendarsId)) {
