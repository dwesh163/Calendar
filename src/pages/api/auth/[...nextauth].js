	...
	callbacks: {
		async signIn({ user, account, profile }) {
			...
			try {
				...
				if (existingUser.length) {
					...
					await connection.execute('INSERT INTO calendars (calendar_name, calendar_id_public, calendar_user_id) VALUES (?, ?, ?)', ['Calendar 1', uuidv4(), userId]);
				}
		...
		async session({ session, token, user }) {
			...
			try {
				...
				if (existingUser.length) {
					var token = jwt.sign({ time: new Date().getTime(), id: existingUser[0].user_id_public, key: existingUser[0].user_key_private, calendarId: calendarsId.map((item) => item.calendar_id_public) }, existingUser[0].user_key_public);
